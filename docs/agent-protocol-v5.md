# Agent Protocol 5.0.0

이 파일 경로의 `v5`는 안정적인 major 계약을 뜻한다. 정확한 현재 프로토콜
...
13. 전역 에이전트 신원과 연결 키는 워크스페이스 grant와 분리된다. 같은 에이전트와 키라도
    워크스페이스마다 접근 프로필·capability·문서 범위·변경 커서가 다르며, 키의 명시적 활성
    binding과 grant가 모두 있어야 접근할 수 있다.
...
Nyxdoc는 문서 내용의 비밀 패턴을 임의 검사·차단하지 않으며, 인증·워크스페이스 grant
capability·연결 키 binding과 scope·문서 범위로 권한 없는 접근을 통제한다.
