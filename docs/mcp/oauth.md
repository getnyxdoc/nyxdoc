...
워크스페이스에 할당되어 있으면 기존 역할과 세부 권한을 보존하고, 아직 할당되지 않은
워크스페이스에만 승인 화면의 역할을 적용한다.
...
- 토큰만으로 워크스페이스 멤버십이나 문서 범위를 우회할 수 없다.
...
기존 Bearer 연결 키는 하위 호환 경로로 유지한다. OAuth 연결도 에이전트 관리 화면의 전역
에이전트와 내부 연결 자격으로 보이며, 비활성화·폐기하면 더 이상 MCP 인증에 사용할 수 없다.
워크스페이스별 역할과 문서 범위는 워크스페이스 설정에서 관리한다.
...
- OAuth 승인 UI는 워크스페이스 역할 프리셋을 선택한다. 문서 하위 트리와 세부 permission
  override는 연결 후 워크스페이스 설정에서 조정한다.
